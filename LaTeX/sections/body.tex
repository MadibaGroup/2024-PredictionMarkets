...
\gap{Application of accuracy metrics~\cite{GN07} to various \cepm and \depm platforms. Correlations between accuracy and the market microstructure options in Table~\ref{tab:class}.}
...
\gap{Can market topics be written with machine-checkable predicate specifications (precedence rules, ranked sources, time semantics, and default outcomes), where model checking (\cf~\cite{Cla21}) could eliminate ambiguities, corner cases, and loopholes?}
...
\gap{How does a \textit{negRisk} gadget encourage price parity between equivalent positions? Quantify its improvements for price execution and trade execution costs. Does a reverse gadget, for converting a single yes share, offer similar benefits?}
...
\gap{Measurements of MEV extraction for on-chain \depms. Mechanisms for order-flow privacy for on-chain \depms.}
