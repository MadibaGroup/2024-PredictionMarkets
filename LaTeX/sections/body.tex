...
Due to page restrictions, we relegate the designs for the underlying infrastructure to the full paper. It describes various blockchain options and notes that the transparency of a blockchain can lead to copy-trading, posing a research gap related to that. 
...
Due to page restrictions, we relegate the designs for the market topic to the full paper. It describes the differences between permissioned and permissionless systems for setting up what the market is about. Permissioned topic selection requires a central party, while permissionless designs can have problems with unethical markets (\eg assassination markets), spam, fragmentation, and poorly defined topics. The full paper also provides important real-world examples of ill-defined market topics that have proven to be problematic. It provides an insight and a research gap related to these points. 
...
\paragraph{Example.} Alice owns 1000 No shares for Adam Back, Hal Finney, and Nick Szabo in a market about an HBO documentary set to identify Satoshi Nakamoto (see full paper). Since only one share type can win, Alice is guaranteed at least \$2000 from her positions at resolution (and up to \$3000 is she is right about all three). It is capital inefficient to keep her \$2000 locked up in her portfolio of No shares. NegRisk allows her to converts her position into 1000 Yes shares for every other candidate and extract the \$2000 in cash, maintaining the exact same exposure to the market (\$1000 at risk). 



\subsubsection{Scalar.} A third structure is provided in the full paper. It is not frequently used. 
...
The third approach, \textit{matching}, was used by InTrade~\cite{intrade} and is used today by Kalshi~\cite{kalshi_docs}. We provide a description and research insight in the full paper.
...
Due to page restrictions, we relegate the designs for archiving markets to the full paper. It describes how a market can be reconstructed after the fact, through a mix of on-chain data and off-chain data collected by the operator and possibly offered through APIs. It also presents a research gap related to this.
